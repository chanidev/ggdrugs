...
- **명명 정책**:
  ...
  - **자치구가 있는 일반시**: "수원시 영통구" 처럼 합성형으로 시드. 대상 10개 시: 수원·성남·고양·용인·청주·천안·전주·포항·안산·창원. 컬럼 추가 없이 합성 표기로 검색 매칭 일관성 유지.
  - **자치구 있는 일반시의 시 단위 row**: 위 10개 시는 자치구 row 외에 **"수원시" 같은 시 단위 row 1개씩 추가 시드** (sigungu_name="수원시", dong_name=NULL). 사용자가 "수원시 전체" 를 1단계에서 고르거나 주소에서 자치구 추출 실패한 경우의 fallback 용도. resolver 우선순위: (sido, "수원시 영통구") → (sido, "수원시") → (sido, NULL).
