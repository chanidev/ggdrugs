...
### Phase 1 진입 시 — 완료 (2026-04)

- [x] `apps/bff/src/lib/s3.ts` — `@aws-sdk/client-s3` 기반 S3/MinIO 어댑터. 버킷 ensure 로직 포함.
- [x] `services/llm/openai_chain.py` — OpenAI 클라이언트 + 임베딩/채팅 어댑터.
- [x] `services/llm/qdrant_events.py` — 이벤트 컬렉션. 기사·리뷰 컬렉션은 필요 시 추가.
- [x] `packages/config/src/schema.ts` — `S3_*` · `OPENAI_*` zod 검증 (S3 네이밍으로 MinIO/S3 양쪽 수용).
