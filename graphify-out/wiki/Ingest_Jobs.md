...
- **resolveSeoulRegionId()** (3 connections) — `apps\bff\src\jobs\ingest-common.ts`
- **runKcisaIngest()** (3 connections) — `apps\bff\src\jobs\kcisa-ingest.ts`
- **runSeoulCultureIngest()** (3 connections) — `apps\bff\src\jobs\seoul-culture-ingest.ts`
- **runTourapiIngest()** (3 connections) — `apps\bff\src\jobs\tourapi-ingest.ts`
- **existsInOtherOrigin()** (2 connections) — `apps\bff\src\jobs\ingest-common.ts`
- **extractSeoulGu()** (2 connections) — `apps\bff\src\jobs\ingest-common.ts`
- **getCategoryId()** (2 connections) — `apps\bff\src\jobs\ingest-common.ts`
- **isForwardLooking()** (2 connections) — `apps\bff\src\jobs\ingest-common.ts`
