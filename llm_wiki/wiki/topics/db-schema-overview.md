...
PostgreSQL 20 테이블 구성. 크게 5개 도메인 그룹으로 나뉜다: 사용자·역할 / 이벤트 코어 / 승인 흐름 / 콘텐츠 상호작용 / LLM·크롤링 인프라. extensions는 postgis, postgis_topology, pg_trgm, unaccent, citext (`infra/db/init/01-postgis.sql` 참조). pgvector는 ADR 0002로 **미도입** 확정 (벡터 검색은 Qdrant 단일 사용).

> 📌 아래 테이블명/컬럼명은 **ADR 0001 적용 후 확정본** 기준. DDL v3 원본과의 차이는 각 항목의 "← DDL v3:" 주석으로 표기. ADR 0001 #6(`review_photos` 신설), #7(`event_subscriptions` 신설)은 Phase 1 마이그레이션에서 20 → **22 테이블**로 증가 예정.

## Key points

### 1. 사용자·역할 (3개 → ADR 0001 후 4개)
- **regions** — 행정구역 마스터 (시/도·시/군/구·읍/면/동).
- **users** — OAuth 소셜 로그인 계정. `auth_provider IN (google, kakao)`. soft delete. **+ `active_role VARCHAR(20) NOT NULL DEFAULT 'user'`** (ADR 0001 #2).
...
- **admin_profiles** *(신설, ADR 0001 #3)* — users 1:1 확장. `scope IN ('full', 'content_only', 'uploader_review_only')`.

### 2. 이벤트 코어 (4개)
- **event_categories** — 종류 마스터 (festival/expo/symposium/conference).
...
- **approval_logs** — 관리자 액션 감사 로그. `action IN (approved, revision_requested, rejected)` (← DDL v3: `on_hold`), reason 포함.
