...
2. 모바일(≤640px) 대응 미구현. rail + panel 을 바텀시트로 전환해야 함. Phase 2 후보.
3. Kakao 클러스터러 아직 연결 안 됨. 더미 마커 3개만 렌더. DESIGN.md 의 signature motion (핀 클러스터 분해 stagger 50ms) 구현 대기.
4. EventList 는 더미 3건. 실제 `/events` API (= Phase 2 크롤러 결과) 연동 대기.
