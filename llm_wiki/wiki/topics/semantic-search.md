...
- ~~**PostGIS geom 전환**~~ → **stage 1+2 ship** (2026-04-26 v4.3): `events.location_geom geometry(Point, 4326)` 추가 + 4186/4188 backfill + GiST 인덱스 (`idx_events_location_geom`). BFF `/events?bbox=minLng,minLat,maxLng,maxLat` 신설 — `ST_Within(location_geom, ST_MakeEnvelope(...,4326))` 으로 viewport 필터. 기존 lat/lng column 은 dual-write 유지 (Web/BFF 코드 회귀 0). 마이그레이션 `20260426171500_events_location_geom_postgis`. 검증: Seoul bbox (37.4-37.7 / 126.8-127.1) → 3964 events, Pacific (30-31 / 140-141) → 0. 후속 stage 3 (Web SeoulMap `bounds_changed` listener → bbox refetch) 은 panning UX 결정 트리거 대기로 분리.
- 후속 (v5 후보 잔여):
  - **PostGIS stage 3 — Web viewport hook**: Kakao Maps `bounds_changed` → debounce 300ms → `/events?bbox=...` refetch. 현재는 모든 approved events pre-fetch 패턴 — UX 결정 (panning fetch 빈도, idle threshold) 트리거 대기.
  - **PostGIS stage 4 — lat/lng 컬럼 DROP**: stage 3 ship 후 충분한 검증 기간 (1+ sprint) 거쳐 dual-write 종료 + lat/lng DROP. 마이그레이션 별도.
