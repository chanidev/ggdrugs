...
- ~~**거리순 정렬 (sort=distance)**~~ → **해소** (2026-04-26 v4.5 + v4.6 region anchor): BFF `/events?sort=distance&bbox=...` 또는 `&anchor=lng,lat` 신설. v4.6 — 단일 `regionIds` 필터 + sort=distance 시 region centroid (구청 좌표) 자동 anchor. anchor priority = explicit > region centroid (regionIds 단일) > bbox center > **400 reject**. PostGIS KNN `<->` (GiST 인덱스 활용) ORDER BY + `ST_Distance(geography)` 미터 단위 거리값 응답에 `distanceMeters` 첨부. 2-pass 구현 (Pass A: candidate eventIds, Pass B: KNN 정렬 + distance, Pass C: select 필드 + reorder). EventList 카드에 거리 라벨 (`< 1000m → "%dm"` / `>= 1000m → "%.1fkm"`) 표시. FullListPanel segmented control '거리' 옵션 + mapBbox null 시 disabled. `effective sort` fallback (saved=distance && !mapBbox → fetch 'ending'). SeoulMap → AppShell/MobileShell → FullListPanel prop chain 으로 mapBbox lift up. v1 scope: 지도 center anchor only — GPS / Sigungu 중심 / Kakao Places 는 v5 후보.
- 후속 (v5 후보 잔여):
  - **PostGIS stage 4 — lat/lng 컬럼 DROP**: stage 3 검증 충분 기간 (1+ sprint) 거쳐 dual-write 종료 + lat/lng DROP. 응답 형식 변경 필요 (lat/lng 또는 GeoJSON Point) — Web 변경 동반.
  - **거리순 anchor 확장** — 사용자 GPS (Geolocation API + opt-in 버튼) / Sigungu centroid (Region 모델 마이그레이션) / Kakao Places 검색.
