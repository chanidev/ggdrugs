...
1. **개인화 컨텍스트 빌드** — `resolveAuth` 미들웨어로 로그인 user 면 `user_taste_profiles` 조회 → preferred_category / region / vibe 라벨 (3 dim — recommendations.md / db-schema-overview.md 정본) + 최근 30d 북마크 수 → LLM `user_signals` 로 전달.
...
> 참고: 이 22건은 2026-04-25 시점 기록값. 이후 Slice B/C 에서 suite 가 42건으로 확장됨 — [ai-enrichment.md](ai-enrichment.md) Slice B/C 참조.
...
- ~~**Streaming reconnect**~~ → **해소** (2026-04-26 v4.2 + v4.11 idempotent resume): v4.2 sealed-gate auto-retry + v4.11 Last-Event-ID 기반 idempotent resume. v4.11 — server 가 stream 시작 시 UUID streamId 발행 + 모든 SSE event 에 `id: <streamId>:<seq>` line 추가, in-memory cache (TTL 5분, 단일 인스턴스 가정 — Redis 미사용) 에 저장. 재연결 시 client 가 `Last-Event-ID: <streamId>:<seq>` 헤더 전달 → server 가 cache 에서 그 이후 event replay (LLM 재호출 0). v4.2 가 사용자 명시 retry path 라면 v4.11 은 비용 절감 — blip 발생해도 GPT 토큰 추가 청구 없음. cache miss/expired 시 fresh stream 으로 fallback. SSE 시퀀스 (`stream_start → reply_delta × N → meta → reply_sealed → suggestions → [reply_override?] → done`) — stream_start 추가, client 는 silent consume.
...
- ~~**PostGIS stage 4b — lat/lng 컬럼 DROP**~~ → **해소** (2026-04-26 v4.10): events.latitude / longitude DECIMAL 컬럼 + idx_events_geo (btree) + tr_events_sync_location_geom 트리거 + fn_events_sync_location_geom() 함수 모두 DROP. **location_geom 단일 source of truth**. 마이그레이션 `20260426203000_events_drop_lat_lng_columns`. WRITE 경로 (ingest-common.ts, uploader/events.ts) 가 INSERT 후 별도 raw `UPDATE events SET location_geom = ST_SetSRID(...)` 로 채움. READ 경로 (events.ts, event-detail.ts, bookmarks.ts, uploader 상세) 는 `fetchEventCoords(ids)` 헬퍼로 별도 raw query (`SELECT ST_X / ST_Y`) → coords map → mapEventRow 가 응답에 lat/lng 채움. 응답 형식 그대로 유지 — Web 변경 0. 손실 0 (location_geom 에 100% 보존, 롤백 시 ST_X/ST_Y 로 lat/lng 재생성 가능).
- 후속 (v5 후보 잔여):
