...
/**
 * MyPage — A_500 마이페이지 뼈대.
 *
 * 탭: 내 북마크 / 내 리뷰 (캘린더는 후속).
...
export function MyPage() {
  ...
  return (
    <PageShell>
      ...
      {tab === 'bookmarks' ? <BookmarksList /> : <ReviewsList />}
    </PageShell>
  );
}
...
function ReviewCard({ item }: { item: MyReviewItem }) {
  ...
  return (
    <Link
      to={`/events/${ev.eventId}`}
      className="flex flex-col gap-2 rounded-(--radius-lg) border border-(--color-border) bg-(--color-surface) p-4 transition-colors hover:border-(--color-border-hover) hover:bg-(--color-surface-alt)"
    >
