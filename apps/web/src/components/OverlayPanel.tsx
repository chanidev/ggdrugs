...
export function OverlayPanel({
  ...
  if (!open) return null;
  const title = t(`overlayPanel.${open}`);
  return (
    <section
      ...
      aria-label={t('overlayPanel.detailSuffix', { title })}
    >
      <div className="flex shrink-0 items-center justify-between border-b border-(--color-border) px-5 pb-4 pt-5">
        <h3 className="m-0 text-[18px] font-bold tracking-[-0.015em]">{title}</h3>
        <button
          type="button"
          onClick={onClose}
          aria-label={t('overlayPanel.closeAria')}
